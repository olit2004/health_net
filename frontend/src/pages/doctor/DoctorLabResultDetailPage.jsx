...
export default function DoctorLabResultDetailPage() {
    ...
    useEffect(() => {
        const fetchLabResult = async () => {
            try {
                const response = await api.get(`/doctor/lab-results/${id}`)
    ...
    if (isLoading) {
        return (
            <DoctorLayout title="Clinical Analytics" subtitle="Decrypting laboratory data...">
                <div className="flex flex-col items-center justify-center p-20 space-y-4">
                    <Loader2 className="h-10 w-10 animate-spin text-primary" />
                    <p className="text-muted-foreground font-medium">Accessing secure storage...</p>
                </div>
            </DoctorLayout>
        )
    }
    ...
    if (error || !labResult) {
        return (
            <DoctorLayout title="Access Error" subtitle="Registry integration failure">
                <div className="max-w-md mx-auto text-center space-y-4 mt-20 p-8 bg-destructive/5 rounded-3xl border-2 border-dashed border-destructive/20">
                    ...
                    <p className="text-muted-foreground">{error || "The requested lab result could not be located in the national registry."}</p>
                    <Button onClick={() => navigate(-1)} variant="outline" className="mt-4 border-2">
                        <ChevronLeft className="mr-2 h-4 w-4" /> Return to Profile
                    </Button>
                </div>
            </DoctorLayout>
        )
            ...
            <div className="max-w-5xl mx-auto space-y-8 pb-20">
                ...
                <div className="flex items-center justify-between">
                    ...
                    >
                        ...
                        <span className="font-bold text-lg">Back to Patient Profile</span>
                    </Button>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground bg-muted/50 px-4 py-1.5 rounded-full border">
                        ...
                        Verified Clinical Data • Audit Logged
                    </div>
                </div>
                            ...
                            <div className="flex justify-between items-start gap-6">
                                <div className="space-y-2">
                                    ...
                                    <CardDescription className="text-base font-medium">
                                        Clinical diagnostic report generated for national medical history.
                                    </CardDescription>
                                </div>
                                <div className="text-right">
                                    <Badge
                                        className={cn(
                                            ...
                                            labResult.result_value === 'NORMAL' ? 'bg-green-500/10 text-green-600' : 'bg-destructive/10 text-destructive'
                                        )}
                                    >
                                        {labResult.result_value}
                                    </Badge>
                                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest opacity-60">Result Status: {labResult.status || 'FINAL'}</p>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-8 space-y-10">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 border-b pb-2">
                                    ...
                                    <h3 className="font-black uppercase tracking-widest text-xs text-foreground">Findings Summary</h3>
                                </div>
                                <div className="p-6 bg-muted/20 rounded-2xl border-2 border-dashed border-border/50">
                                    <p className="text-lg font-medium leading-relaxed whitespace-pre-wrap italic text-foreground/80">
                                        {labResult.findings || "No specific analytical findings reported for this test sequence."}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 border-b pb-2">
                                    ...
                                    <h3 className="font-black uppercase tracking-widest text-xs text-foreground">Internal Clinical Notes</h3>
                                </div>
                                <div className="p-6 bg-primary/5 rounded-2xl border-2 border-primary/10">
                                    <p className="font-bold text-muted-foreground whitespace-pre-wrap">
                                        {labResult.notes || "No internal staff notes associated with this entry."}
                                    </p>
                                </div>
                            </div>
                        </CardContent>
                                    ...
                                    <div className="space-y-4 pt-3">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-bold text-sm">Dr. {labResult.doctor?.user?.name || "System Automated Verification"}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <CardContent className="p-6">
                                <div className="flex items-center justify-between mb-6">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-[10px] text-muted-foreground font-medium uppercase underline">Secure Transmission</p>
                                    </div>
